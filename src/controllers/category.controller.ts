...
export const CategoryController = {
    ...
    getAll: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const { rows, total } = await CategoryService.findByUser(userId, { offset, limit }, orgId);
            res.status(200).json({
                data: rows,
    ...
    getById: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const userId = req.user.id;
            const orgId = req.user.organizationId;
            const category = await CategoryService.findByIdAndUser(id, userId, orgId);
    ...
    update: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const updated = await CategoryService.update(id, userId, updateData, orgId);
    ...
    delete: async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const userId = req.user.id;
            const orgId = req.user.organizationId;
            await CategoryService.delete(id, userId, orgId);
